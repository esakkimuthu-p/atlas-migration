...
pub struct CustomerDiscount1 {
    pub id: Thing,
    pub disc: AmountInfo,
}

#[derive(Debug, Serialize)]
pub struct PriceConfig1 {
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub s_customer_disc: Option<Vec<CustomerDiscount1>>,
}

#[derive(Debug, Serialize)]
pub struct InventoryBranchDetail1 {
    pub branch: Thing,
    pub inventory: Thing,
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub price_config: Option<PriceConfig1>,
}
...
fn find_options_advance(projection: Document, skip: u64, limit: i64) -> FindOptions {
    FindOptions::builder()
        .sort(doc! {"_id": 1})
...
impl Inventory1 {
    pub async fn create(surrealdb: &Surreal<SurrealClient>, mongodb: &Database) {
        ...
        let mut skip = 31264;
        let limit = 1000;
        println!("Total inventory count \x1b[1;28m{}\x1b[0m", count);
        let mut run_count = count / limit;
        if (run_count * limit) < count {
            run_count += 1
        };
        for _x in 1..=run_count {
            println!("{} to {} of {} start", skip, skip + limit, count);
            let find_opts = find_options_advance(
                doc! {"createdAt": 0, "updatedAt": 0, "createdBy": 0, "updatedBy": 0},
                skip,
                limit as i64,
            );
            ...
            for d in docs {
                ...
                for u_item in d.get_array_document("units").unwrap_or_default() {
                    ...
                    if unit.conversion == 1.0 {
                        ...
                        .iter()
                        .map(|x: &InventoryUnit1| x.conversion)
                        .collect::<Vec<f64>>()
                ...
                if let Some(branch_details) = d.get_array_document("branchDetails") {
                    for br_detail in branch_details {
                        ...
                        {
                            price_config = Some(PriceConfig1 {
                                s_disc,
                                cost_margin,
                                mrp_margin,
                                min_profit_margin,
                        ...
                        let _created: Created = surrealdb
                            ...
                            .content(InventoryBranchDetail1 {
            ...
            println!("{} to {} of {} end", skip, skip + limit, count);
            skip += limit;
        }
