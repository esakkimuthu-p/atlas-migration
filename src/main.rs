...
use model::{
    Account, AccountTransaction, Batch, Inventory, InventoryCess, InventoryTransaction,
    InventoryUnit,
};
...
async fn main() {
    ...
    DB.query("DEFINE INDEX account_type ON TABLE account COLUMNS account_type")
        .await
        .unwrap()
        .take::<Option<()>>(0)
        .unwrap();
    DB.query("DEFINE INDEX val_name ON TABLE account COLUMNS val_name")
        .await
        .unwrap()
        .take::<Option<()>>(0)
        .unwrap();
